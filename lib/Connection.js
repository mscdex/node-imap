...
Connection.prototype.setAcl = function(box, identifier, perms, cb) {
  if (!this.serverSupports('ACL'))
    throw new Error('Server does not support ACL');

  this._enqueue('SETACL "' + escape(utf7.encode(''+box)) + '" "' + identifier + '" "' + perms + '"', cb);
};

Connection.prototype.getAcl = function(box, cb) {
  if (!this.serverSupports('ACL'))
    throw new Error('Server does not support ACL');

  this._enqueue('GETACL "' + escape(utf7.encode(''+box)) + '"', cb);
};
...
Connection.prototype._resUntagged = function(info) {
  ...
  if (type === 'bye')
    ...
    for (var i = 1; i < list.length; i += 2)
      rights.push({ name: list[i], perms: list[i + 1] });
    this._curReq.cbargs.push({ box: list[0], rights: rights });
  } else if (type === 'status') {
    box = {
      name: info.text.name,
      uidnext: 0,
