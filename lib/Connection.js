...
Connection.prototype._fetch = function(which, uids, options) {
  ...
  if (this.serverSupports('X-GM-EXT-1') && options.gmail !== false) {
    fetching.push('X-GM-THRID');
    fetching.push('X-GM-MSGID');
    fetching.push('X-GM-LABELS');
  }
  if (this.serverSupports('CONDSTORE') && !this._box.nomodseq && options.modseq !== false)
...
Connection.prototype._resUntagged = function(info) {
  ...
  if (type === 'bye')
    ...
    if (/^(?:UID )?FETCH/.test(this._curReq.fullcmd)) {
      ...
      //destel: workaround, if we didn't get attributes from list below... then let's pretend we we're not going to fetch them
      //otherwise condition below is not matched and 'attributes' and 'end' events are not emitted
      var ignored = ['UID']
      toget = toget.filter(function(v) {
        return !v || !v.toUpperCase || ignored.indexOf(v.toUpperCase()) == -1
      })
