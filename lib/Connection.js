...
Connection.prototype._resTagged = function(info) {
  ...
  if (req.bodyEmitter) {
    ...
    process.nextTick(function() {
      ...
      //destel: emit 'attributes' and 'end' events for messages which are not yet ended
      for (var sn in req.fetchCache) {
        if (req.fetchCache.hasOwnProperty(sn)) {
          var item = req.fetchCache[sn]
          if (!item.ended) {
            item.ended = true
            item.msgEmitter.emit('attributes', item.attrs);
            item.msgEmitter.emit('end');
          }
        }
      }
      //end
