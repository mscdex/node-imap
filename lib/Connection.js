...
Connection.prototype.connect = function() {
  ...
  parser.on('continue', function(info) {
    ...
    if (!(self._sock && self._sock.writable)) return;
    if (type === 'IDLE') {
...
Connection.prototype._fetch = function(which, uids, options) {
  ...
  fetching.push('UID');
  if(!options || options.flags !== false)
  fetching.push('FLAGS');
  if(!options || options.internalDate !== false)
...
Connection.prototype._resUntagged = function(info) {
  ...
  if (type === 'bye')
    ...
    this._curReq && this._curReq.cbargs.push(info.text);
    ...
    if (info.text.results !== undefined) {
      ...
      this._curReq && this._curReq.cbargs.push(info.text.results);
      this._curReq && this._curReq.cbargs.push(info.text.modseq);
    } else
      this._curReq && this._curReq.cbargs.push(info.text);
  } else if (type === 'quota') {
    ...
    if (this.delimiter === undefined)
      ...
      if (this._curReq && this._curReq.cbargs.length === 0)
      ...
      var name = info.text.name,
          curChildren = this._curReq?this._curReq.cbargs[0]:{};
    ...
    this._curReq && this._curReq.cbargs.push(box);
   } else if (type === 'fetch' && this._curReq) {
    if (/^(?:UID )?FETCH/.test(this._curReq.fullcmd)) {
      ...
      } else if (msg === undefined && this._curReq) {
...
Connection.prototype._starttls = function() {
  ...
  this._enqueue('STARTTLS', function(err) {
    ...
    // Host name may be overridden the tlsOptions
    for (var k in self._config.tlsOptions)
      tlsOptions[k] = self._config.tlsOptions[k];
