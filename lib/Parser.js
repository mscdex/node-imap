...
var CH_LF = 10,
    ...
    RE_LISTCONTENT = /^\((.*)\)$/,
    RE_LITERAL = /\{(\d+)\}$/,
    RE_UNTAGGED = /^\* (?:(OK|NO|BAD|BYE|FLAGS|ID|LIST|XLIST|LSUB|SEARCH|CAPABILITY|NAMESPACE|PREAUTH|SORT|THREAD|ESEARCH|QUOTA|QUOTAROOT)|(\d+) (EXPUNGE|FETCH|RECENT|EXISTS))(?:(?: \[([^\]]+)\])?(?: (.+))?)?$/i,
    RE_STATUS_CMD = /^\* (STATUS) (.+)?$/i,
...
Parser.prototype._resUntagged = function() {
  ...
  if (m = RE_BODYLITERAL.exec(this._buffer)) {
    ...
    this._buffer = this._buffer.replace(RE_LITERAL, LITPLACEHOLDER);
    this._literallen = parseInt(m[1], 10);
  } else if (m = RE_STATUS_CMD.exec(this._buffer)) {
    this._buffer = '';
    // normal single line response

    // m[1] = response type
    // m[2] = response text
    val = parseStatus(m[2], this._literals);
    this._literals = [];
...
function parseBoxList(text, literals) {
  // We need to make sure that the box name in the response is surrounded in
  // quotes
  text = text.replace(RE_LIST_REPONSE, function(match, p1, boxName, offset, str) {
    if (!boxName) {
      return str
    }
    if (boxName[0] === '"' && boxName[boxName.length - 1] === '"') {
      return p1 + boxName
    }
    if (boxName[0] === "'" && boxName[boxName.length - 1] === "'") {
      return p1 + boxName
...
function parseStatus(text, literals) {
  // We need to make sure that the box name in the response is surrounded in
  // quotes
  text = text.replace(RE_STATUS_RESPONSE, function(match, boxName, p2, offset, str) {
    if (!boxName) {
      return str
    }
    if (boxName[0] === '"' && boxName[boxName.length - 1] === '"') {
