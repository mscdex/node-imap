...
ImapConnection.prototype._login = function(cb) {
  ...
  if (this._state.status === STATES.NOAUTH) {
    ...
    if (this._serverSupports('AUTH=XOAUTH') && 'xoauth' in this._options) {
      ...
      var clientResponse = 'user=' + this._options.username + '\u0001auth=Bearer ' + this._options.xoauth2 + '\u0001\u0001';
      this._send('AUTHENTICATE XOAUTH2 ' + utils.escape( new Buffer( clientResponse ).toString('base64') ),
                 fnReturn);
