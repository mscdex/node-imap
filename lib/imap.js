...
ImapConnection.prototype.removeDeleted = function(uids, cb) {
  ...
  if (uids !== undefined) {
    ...
    this._send('UID EXPUNGE ' + uids.join(','), cb);
  } else
...
ImapConnection.prototype._move = function(which, uids, boxTo, cb) {
  ...
  if (this.serverSupports('MOVE')) {
    ...
    this._send(which + 'MOVE ' + uids.join(',') + ' "'
               + utils.escape(utf7.encode(''+boxTo)) + '"', cb);
  } else if (this._state.box.permFlags.indexOf('deleted') === -1) {
