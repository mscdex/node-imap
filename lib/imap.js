...
var CRLF = '\r\n',
    ...
    RE_CMD = /^([A-Za-z]+)(?: |$)/i,
    REX_UNRESPDATA = XRegExp('^\\* (?:(?:(?<type>NAMESPACE) (?<personal>(?:NIL|\\((?:\\(.+\\))+\\))) (?<other>(?:NIL|\\((?:\\(.+\\))+\\))) (?<shared>(?:NIL|\\((?:\\(.+\\))+\\))))|(?:(?<type>FLAGS) \\((?<flags>.*)\\))|(?:(?<type>LIST|LSUB|XLIST) \\((?<flags>.*)\\) (?<delimiter>".+"|NIL) (?<mailbox>.+))|(?:(?<type>SEARCH)(?: (?<results>.+))?)|(?:(?<type>STATUS) (?<mailbox>.+) \\((?<attributes>.*)\\))|(?:(?<type>CAPABILITY) (?<capabilities>.+))|(?:(?<type>BYE) (?:\\[(?<code>.+)\\] )?(?<message>.+)))$', 'i'),
      ...
      if (m = XRegExp.exec(ondata.line, REX_UNRESPNUM)) {
        ...
        switch (m.type) {
          ...
          case 'XLIST':
            ...
            m.delimiter = parsers.convStr(m.mailbox, ondata.literals);
        ...
        switch (m[1]) {
          case 'OK':
            if (state.status === STATES.NOAUTH) {
              ...
              var r;
              if (r = /^UIDVALIDITY (\d+)/i.exec(m[2]))
                state.box.uidvalidity = r[1];
              else if (r = /^UIDNEXT (\d+)/i.exec(m[2]))
                state.box.uidnext = r[1];
              else if (r = /^PERMANENTFLAGS \((.*)\)/i.exec(m[2])) {
                var idx, permFlags, keywords;
                state.box.permFlags = permFlags = r[1].split(' ');
