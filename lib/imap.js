...
// private function that modifies the ImapMessage
var _mod_message = function(msg) {
    msg.stream = ImapMessageStream(msg);
...
ImapConnection.prototype.connect = function(loginCb) {
  ...
  function emitLitMsg(key, msg) {
    ...
    for (var i=0, len=fetches.length; i<len; ++i) {
      if (!fetches[i]._msg) {
        fetches[i]._msg = msg;
        fetches[i].emit('message', _mod_message(msg));
      }
    }
  }
            ...
            if (isUnsolicited)
              ...
              for (body = 0, lenb = bodies.length; body < lenb; body += 2) {
                ...
                for (var i=0, len=fetches.length; i<len; ++i) {
                  ...
                  if (shouldEmit)
                    fetches[i].emit('message', _mod_message(fetches[i]._msg));
