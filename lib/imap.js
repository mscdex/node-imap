...
ImapConnection.prototype.connect = function(loginCb) {
  ...
  function ondata(b) {
    ...
    if (indata.line[0] === '*') { // Untagged server response
      // trim spaces at the end of indata.line to avoid errors when the server
      // sends a non trimmed response (appens
      // with yahoo when your search has no results)
      indata.line = indata.line.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
