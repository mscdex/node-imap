...
var CRLF = '\r\n',
    ...
    RE_CMD = /^([^ ]+)(?: |$)/,
    RE_ISHEADER = /HEADER/,
    REX_UNRESPDATA = XRegExp('^\\* (?:(?:(?<type>NAMESPACE) (?<personal>(?:NIL|\\((?:\\(.+\\))+\\))) (?<other>(?:NIL|\\((?:\\(.+\\))+\\))) (?<shared>(?:NIL|\\((?:\\(.+\\))+\\))))|(?:(?<type>FLAGS) \\((?<flags>.*)\\))|(?:(?<type>LIST|LSUB|XLIST) \\((?<flags>.*)\\) (?<delimiter>"[^"]+"|NIL) (?<mailbox>.+))|(?:(?<type>(SEARCH|SORT))(?: (?<results>.*))?)|(?:(?<type>STATUS) (?<mailbox>.+) \\((?<attributes>.*)\\))|(?:(?<type>QUOTAROOT) (?<mailbox>"[^"]+"|\\S+)(?<quotaroots>.*))|(?:(?<type>QUOTA) (?<quotaroot>.+) \\((?<resources>.*\\)))|(?:(?<type>CAPABILITY) (?<capabilities>.+))|(?:(?<type>BYE) (?:\\[(?<code>.+)\\] )?(?<message>.+)))[ \t]*(?:$|\r\n)', 'i'),
    ...
    if (indata.line[0] === '*') { // Untagged server response
      ...
      if (m = XRegExp.exec(indata.line, REX_UNRESPNUM)) {
        ...
        switch (m.type) {
          ...
          case 'QUOTAROOT':
            m.mailbox = utf7.decode(''+parsers.convStr(m.mailbox, indata.literals));
            m.quotaroots = parsers.parseExpr(m.quotaroots.trim(), indata.literals);
            if (requests[0].cbargs.length === 0)
              requests[0].cbargs.push({});
            if (m.quotaroots) {
              m.quotaroots.forEach(function(quotaroot) {
                requests[0].cbargs[0][quotaroot] = {};
              });
            }
            break;
          case 'QUOTA':
            m.quotaroot = parsers.convStr(m.quotaroot, indata.literals);
            var ret = {};
            if (m.resources) {
              m.resources = parsers.parseExpr(m.resources, indata.literals);
              for (i=0,len=m.resources.length; i<len; ++i) {
                ret[m.resources[i].toLowerCase()] = {
                  usage: parseInt(m.resources[++i], 10),
                  limit: parseInt(m.resources[++i], 10)
                }
              }
            }
            // QUOTA response from GETQUOTAROOT
            if (requests[0].cbargs.length)
              requests[0].cbargs[0][m.quotaroot] = ret;
            // QUOTA response from SETQUOTA/GETQUOTA
            else
              requests[0].cbargs.push(ret);
            break;
          case 'CAPABILITY':
...
ImapConnection.prototype.getQuotaRoot = function(name, cb) {
  if (!this.serverSupports('QUOTA'))
    return cb(new Error('Quota is not supported on the server'));
  this._send('GETQUOTAROOT "' + utils.escape(utf7.encode(''+name)) + '"', cb);
};

ImapConnection.prototype.getQuota = function(quotaroot, cb) {
  if (!this.serverSupports('QUOTA'))
    return cb(new Error('Quota is not supported on the server'));
  this._send('GETQUOTA "' + utils.escape(utf7.encode(''+quotaroot)) + '"', cb);
};
