...
ImapConnection.prototype.connect = function(loginCb) {
  ...
  if (this._options.secure) {
    ...
    state.conn = tls.connect(this._options.port, this._options.host, function(){
    ...
    state.conn.on('secureConnect', function() {
      state.connected = true;
      self.debug&&self.debug('[connection] Secure connection made.');
    });
  } else {
    state.conn = net.createConnection(this._options.port, this._options.host, function(){
        state.connected = true;
        state.authenticated = false;
        self.debug&&self.debug('[connection] Connected to host.');
        state.conn.write('');
  ...
  state.conn.on('error', errorHandler);
  ...
  if (this._state.status === STATES.NOAUTH) {
    ...
    if (this._serverSupports('AUTH=XOAUTH') && this._options.xoauth) {
      ...
      this._send('AUTHENTICATE XOAUTH2 ' + utils.escape(this._options.xoauth2),
                 fnReturn);
    } else if (this._options.username !== ''
